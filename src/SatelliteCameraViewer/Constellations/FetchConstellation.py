...
class FetchConstellation:
    ...
    _DIRECTORY = 'data/constellation'
    _DIRECTORY = '../../spel-uchile/Star_Tracker/py3/data/constellation'
    ...
    def __init__(self, name, min_visibility=None):
        ...
        if min_visibility:
            ...
            self._min_visibility = self.MIN_VISIBILITY

        self._name = name
        self._path = Path(self._DIRECTORY) / (self._name + '.txt')
        if not self._path.is_file():
            raise FetchConstellationError('%s: %s file not found' % (self._name, self._path)) from None
        self._s = None
